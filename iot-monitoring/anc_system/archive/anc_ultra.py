...
# 좋은 위상 기록
good_phases = deque(maxlen=20)  # 성공했던 위상들 기억
...
def main():
    ...
    while is_running:
        ...
        # 1. 게인 조절 (덜 급격하게)
        if reduction > 6:  # 아주 심한 악화만
            gain_multiplier = max(0.4, gain_multiplier * 0.75)
        elif reduction > 3:
            gain_multiplier = max(0.5, gain_multiplier * 0.85)
        elif reduction < -3:  # 잘 작동
            gain_multiplier = min(1.2, gain_multiplier * 1.08)
        elif reduction < SUCCESS_THRESHOLD:  # 성공
            gain_multiplier = min(1.15, gain_multiplier * 1.04)
        elif reduction < 0:  # 약간 효과
            gain_multiplier = min(1.1, gain_multiplier * 1.02)
        
        # 2. 위상 조정 (좋았던 위상 우선!)
        if bad_streak >= 2:
            # 먼저 좋았던 위상들 시도
            if len(good_phases) > 3:
                recent_good = list(good_phases)[-5:]
                avg_good_offset = np.mean(recent_good)
                
                # 좋았던 위상 근처에서 탐색
                test_offsets = [
                    avg_good_offset,
                    avg_good_offset + np.radians(10),
                    avg_good_offset - np.radians(10),
                    phase_offset + np.radians(15),
                    phase_offset - np.radians(15),
                ]
            else:
                search_range = np.radians(20 + bad_streak * 5)
                search_range = min(search_range, np.radians(50))
            ...
            for test_offset in test_offsets:
                ...
                if test_error < best_test_error * 0.92:  # 8% 개선
        ...
        if good_streak >= 4:
            phase_offset *= 0.92
            gain_multiplier = min(1.0, gain_multiplier + 0.03)
        ...
        print(f"{status} | 원:{orig:5.0f}→에:{error:5.0f} | "
              ...
              f"{success_rate:.0f}%({total_good}/{total_count})")
...
def signal_handler(sig, frame):
    ...
    if total_count > 0:
        ...
        result = "🏆 목표 달성!" if rate >= 40 else "📊 결과"
