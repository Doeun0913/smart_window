...
const DataProvider = ({ children }) => {
  ...
  const fetchNoiseData = useCallback(async () => {
    try {
  ...
  }, [fetchCurrentData, fetchTemperatureData, fetchNoiseData, fetchAirQualityData]);
...
const Dashboard = ({ onNavigate }) => {
  const {
    currentData, pldcState, togglePLDC, isLoading, lastUpdate,
    refreshAllData, isConnected, alerts, resolveAlert, error
  } = useApiData();
