#!/usr/bin/env python3
"""
🔒 Lock-On ANC - 최적값 찾으면 고정하고 유지!

-3dB 이상 감소하면 그 설정에 LOCK
"""
...
def main():
    ...
    global lock_gain, lock_phase, consecutive_good, total_good, total_count
    ...
    # 정밀 탐색: 게인 8단계 x 위상 72단계 = 576 조합
    for gain in [0.01, 0.015, 0.02, 0.025, 0.03, 0.035, 0.04, 0.05]:
        current_gain = gain
        gains_best = 999
        gains_best_phase = 0
    ...
    print(f"\n🎯 전체 최적: 게인={best_gain:.3f}, 위상={int(best_phase*180/np.pi)}°, 감소={best_reduction:+.1f}dB")
    ...
    while is_running:
        ...
        orig = get_rms(mic1_buffer, TARGET_FREQ)
        error = get_rms(mic2_buffer, TARGET_FREQ)
        reduction = 20 * np.log10(error / baseline_rms) if baseline_rms > 0.1 else 0
        ...
        print(f"{status} | 원본:{orig:5.0f} → 에러:{error:5.0f} | "
              ...
              f"🔒 G:{lock_gain:.3f} P:{int(lock_phase*180/np.pi)}° | "
